...
| D9 | ~~Stop guards from breaking when code moves~~ — **the mechanism is in; the migration is a ratchet** | **10 Aug.** Nine guards had failed because the code they check moved, and each fix was the same patch — point the path somewhere new — which only set up the next one. `@/test-support/source` locates code by symbol: `sourceDeclaring("RecordDesk")` survives a move because it names the thing rather than its address, and throws by name for a symbol nothing declares rather than silently asserting over an empty string. The scale was larger than the nine suggested — **250 path reads across 50-odd test files**, of which 89 of 96 distinct reads had a unique symbol available, pages included. Eleven files migrated, both that broke among them. The remainder is `path-reading-tests.json`: 44 files may keep their paths, nothing may join, and a file that leaves cannot return — the same shape as the size baseline, and for the same reason, since a 250-site rewrite of the suite that has to stay trustworthy is a worse trade than tightening it as files are opened anyway. **Two name collisions fell out of it**, both invisible while guards addressed code by file: `editTargetFor` meant two different things in two files, and `app/review/expenses/page.tsx` named its lazy wrapper after the panel it loads. You cannot locate code by symbol until symbols are unambiguous, which turns out to be the useful part. | Finish the 44 opportunistically. Backend `.py` reads stay paths on purpose — `sourceDeclaring` scans `src/`, and those guards exist to compare the two sides. |
