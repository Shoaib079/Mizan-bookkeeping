...
| D10 | Finish `classify_statement_line` — 1,424 lines is still one function | **Started 10 Aug: 1,716 → 1,424**, by collapsing the tail twenty-two branches shared. What remains is a 666-line preamble (validation, counterparty resolution, matching, needs-review routing) and 758 lines of flat `if classification == …` that wants to be a dispatch table. **Collapsing the tail is what found the two drifts below**, neither visible while the copies sat 300 lines apart — which is the argument for finishing rather than stopping at "it compiles and it is shorter". | The preamble splits along its own seams: resolve, match, route. The branches become `CLASSIFICATION_POSTERS: dict[StatementLineClassification, Poster]`, each returning what `_finish_classified_line` needs. All 25 classifications are covered by tests, which is what makes it safe. |
