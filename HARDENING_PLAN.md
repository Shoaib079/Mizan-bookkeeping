...
| D4 | A write-off cannot be corrected from the General ledger | `/write-offs/{id}/correct` exists, but `gl-entry-actions.tsx` has no case for it, and the fallback it used to take opened a **credit-sale** form that posts to a route rejecting a `DISCOUNT` row. The ledger now declines rather than offering a form that cannot submit. Correcting one works on the customer page, which owns the right form. | A form, not a rule. Add the case and give the capability an `edit_kind`. |
| D5 | Say on the Expenses page that it lists hand-recorded expenses only, and link to the Expense register | "Expenses" means two things. Review → Expenses lists `ExpenseEntry` rows — what was typed in or read off a receipt. Salaries, supplier invoices, delivery commission, bank fees and FX spend all hit expense *accounts* in the GL through their own flows, so the page's total never equals real expenses and the owner goes to Staff to find the difference. Nothing is wrong with the books: the P&L and `expense_register` both read the ledger and both tie. It is a label problem, and the same one as "Period total" and "in this period" — a figure named more narrowly than it is, on a page that looks authoritative. | Not blocked. A sentence and a link; the register already exists and already ties to the P&L. |
