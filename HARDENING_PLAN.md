...
| --- | --- | --- | --- |
| D2 | Let the backend say how many subledger rows share an entry, then migrate the partner and staff pages to the API | Their local rules encode "this row is one of several sharing a journal entry", which the capability table has no way to express — a profit allocation writes one partner row per partner, a salary payment that applied an advance writes two staff rows, a period payment three. Migrating without it would put a Void on Ali's profit row that voids Burak's and Cem's share too. | Not blocked. Cheap now that the table knows each source's owner model: `select(count()).where(owner.journal_entry_id == entry.id)`, one field on the response, one place. Before Phase 2 step 2 it would have meant editing 46 branches. |
| D3 | Voiding a group-sale **discount** from the General ledger voids the whole group sale | `post_group_sale_discount` writes its own journal entry with source `GROUP_SALE` and sets `reference_id` to the sale. The escape rule reads "GROUP_SALE + a reference_id → void at `group-sales/{reference_id}/void`", which is right for the sale and wrong for a discount against it. The route exists, so nothing 404s — it voids the wrong record, which is the failure the table's tests were written to fear. Predates Phase 2. `post_customer_write_off` is unaffected: it sets no `reference_id`. | Needs a decision on what voiding a discount *should* do — reverse just the discount, presumably — and a route for it. |
