...
| D9 | Stop guards from breaking when code moves | Nine source-reading guards have now failed for the wrong reason — the code they check moved to another file. Today alone: `gl-edit-kinds.test.ts` when the edit switch left `gl-entry-actions.tsx`, and `add-page-simplify-guard.test.ts` when `role="tab"` left `record-desk.tsx`. Each failure costs the same minute of believing something real broke, and each fix has been the same patch: point the path somewhere new. The pattern is that they locate code by **file** when what they mean is a **symbol** — "wherever `role=\"tab\"` is written", not "line 372 of this file". | Locate by symbol or scan a directory, so a move is invisible to the check. Every one of the nine also needs the same companion it has now: a `readFileSync` that throws by name, so a stale path fails loudly rather than by matching nothing. |
