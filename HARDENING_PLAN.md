...
| Faults found by building something adjacent, not by looking | 13 — the seven of 9 Aug, plus the 204 idempotency swallow, a 403 that was a 500, an RLS-blind inventory, two name collisions, `classification_source` set by 2 of 22, and `OTHER_INCOME` learning nothing (10 Aug) |
...
- **`BANK_STATEMENT_LINE_REF`** was the same string literal written out twice,
  ...
  to agree for the link to resolve at all. **One home, 10 Aug** — found only
  because the posters moved into their own package and needed to import it
  from somewhere.
- **The tail of twenty-two classification branches**, ~40 lines each, copied.
  Collapsed 10 Aug, and the copies had drifted in two ways. `classification_source`
  was set by two of twenty-two: harmless today, since every reader compares
  against `"rule_auto"` and NULL behaves as `"manual"` — but the natural thing
  for the next person to write is `== "manual"`, and twenty would have fallen
  silently outside it. And `OTHER_INCOME` recorded **no learning at all**, the
  only one of the twenty-two: a learned rule that is not auto-postable still
  pre-labels the next matching line in review, so the app remembered a
  recurring rent *payment* and forgot the rent *receipt* beside it. Neither is
  findable by reading one branch; both are obvious the moment they are one.
...
- **Link keys on a statement line.** `_finish_classified_line` does
  `setattr(line, key, value)`, and `setattr` takes any string: a key that is
  not a column becomes an ordinary Python attribute, the commit succeeds, and
  the counterparty is simply never recorded. Extracting the posters rewrote
  twelve of these keys by mistake and **one of the twelve failed a test** — a
  supplier payment could post with no supplier on the line and the suite stayed
  green. *Now checked at the point of the write*, plus a source scan of the
  poster package.

**The rule.** *If a string must match a route — or a column — a test resolves
it.*
...
**The one thing that has worked.** Break it on purpose, watch it go red, in the
same sitting. Every guard written on 10 Aug was checked this way, and it was
not ceremony: the first mutation aimed at the new render tests *did not fail
them*, because a comment sat between the `case` and the setter and the edit
never applied. Believing that would have meant trusting a test that had never
been shown to bite. Where a test cannot be broken on purpose, it is not a test.
...
| D9 | ~~Stop guards from breaking when code moves~~ — **the mechanism is in; the migration is a ratchet** | **10 Aug.** Nine guards had failed because the code they check moved, and each fix was the same patch — point the path somewhere new — which only set up the next one. `@/test-support/source` locates code by symbol: `sourceDeclaring("RecordDesk")` survives a move because it names the thing rather than its address, and throws by name for a symbol nothing declares rather than silently asserting over an empty string. The scale was larger than the nine suggested — **250 path reads across 50-odd test files**, of which 89 of 96 distinct reads had a unique symbol available, pages included. Eleven files migrated, both that broke among them. The remainder was `path-reading-tests.json` — 32 files, tightened as they were opened. **10 Aug (later): finished.** All 31 migratable files done by hand, one at a time; the baseline file is deleted and the ratchet is now a rule with a single stated exception, `archetype-coverage.test.ts`, which walks `app/` to discover routes — in Next.js a route *is* a path, so naming those pages by symbol would mean writing down the list the guard exists to find. Two verification scripts came out of it: `scripts/unique-symbols.mjs` (which symbols a file offers) and `scripts/check-symbol-guards.mjs` (every `sourceDeclaring("X")` resolves to exactly one file), both copying `source.ts`'s predicate verbatim rather than reimplementing it. The second one first reported "0 unresolved" over a file where it had checked nothing, because it only read literal arguments and `archetypes.test.ts` passes 48 symbols through a loop variable. **One migration changed behaviour and only the suite caught it:** `lists.filter((l) => !l.includes("partners"))` excluded the partners page while the list held paths; once it held components the substring stopped matching `"PartnersPage"` on the capital P, so an exemption silently switched itself off. A filter that excludes nothing is indistinguishable from one that works, and the verification I *did* run — every guard resolves to the same file it used to read — could not see it, because it compared what was read and not what was filtered out. Exceptions are named explicitly now, not matched. **Four name collisions fell out of it**, all invisible while guards addressed code by file: `editTargetFor` meant two different things in two files, and three `dynamic()` wrappers were named after the panels they load (`ExpensesReviewPanel`, then `InvoicesReviewPanel` and `ReceiptsReviewPanel`, found only when the migration needed to name them). You cannot locate code by symbol until symbols are unambiguous, which turns out to be the useful part. | Done. Backend `.py` reads stay paths on purpose — `sourceDeclaring` scans `src/`, and those guards exist to compare the two sides. |
| ~~D10~~ | ~~Finish `classify_statement_line`~~ | **Done 10 Aug. 1,716 → 757 lines**, and `statements.py` 3,098 → 1,939. The twenty-two branches are named posters in `statement_posters/`, grouped by what they touch — payments settle a record that already exists, expenses have no counterparty ledger, staff and partners each write their own id, loans are the bank's own — dispatched from a table that `test_classification_poster_table.py` compares against the enum. `statement_classify_core.py` holds what happens to a line after a posting succeeds. Every poster body was verified byte-identical to the branch it replaced. **What the split turned up:** `BANK_STATEMENT_LINE_REF` was written out twice as the same literal, in `statements.py` and `statement_rule_auto.py` — Class 1, invisible until the posters needed to import it from somewhere. Plus the two drifts the tail collapse had already found. | The 666-line preamble is still one block: validation, counterparty resolution, matching, needs-review routing. Three separable jobs, and unlike the branches they are not copies of each other. |
...
| ~~D6~~ | ~~Collapse `gl-entry-actions.tsx`'s ten `useState` pairs into one edit target, and move its twelve dialog render blocks to a sibling~~ | **Done 10 Aug. 532 → 154 lines**, and the first file to come back off the size baseline. It was blocked for a day on the real reason, not a stylistic one: nothing could catch "the dialog no longer opens". `tsc` accepts a refactor that wires `partner_ledger` to the staff form — both are objects of the right shape — and `gl-edit-kinds.test.ts` only compares two lists of strings read from source. So the harness came first: jsdom, Testing Library, and `gl-entry-actions.render.test.tsx`, which presses Edit for all fourteen kinds with every child form replaced by a marker, then asserts the right one opened **and none of the others did**. Those tests were written against the old code, made to pass there, and deliberately broken twice — a case wired to the wrong setter, a dialog dropped from the tree — to prove they could fail. Then the refactor, and they passed unchanged. Now: one `editTarget`, one pure `editTargetFor()` in `lib/`, one `<GlEditDialogs>` sibling. The twelve `&&` blocks could each render independently, so two dialogs mounting at once was prevented only by convention; a `switch` that returns makes it structural. | — |
