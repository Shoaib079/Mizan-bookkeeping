...
- **Payment returned / bounced (bounce pair)** — *owner 2026-09-01.* **DONE (`v0.statement-bounce-pair`).** Pair outflow + return (+ optional bank fee); supplier/staff/partner; void payment first if posted.
- **Statement classify bar slim-down** — *owner 2026-09-01.* The post bar (`StatementClassifyBar` + picker) feels oversized. Audit notes: ~22 classifications in one searchable list; target Combobox often **empty** when type needs no target (`bank_fee`, `pos_settlement`, `loan_*`, `unknown`); permanent “Learn as” + bank-fee tip row; dual UIs (banking classify bar vs Review row actions). Slim layout / progressive disclosure / hide empty target — **separate UX slice**, not bounce.
