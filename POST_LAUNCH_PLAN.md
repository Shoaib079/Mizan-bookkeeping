...
- **Period close — month + year (2026-07-27, `v0.month-close-slice-1` / `v0.month-close-snapshot`)** — the lock engine already existed and had no UI. Added **Reports → Month close**: a readiness checklist (unclassified bank lines **BLOCK**; card-clearing residual, uncounted drawers, over/short days, missing bank closing balances, staff with no salary **warn**), close/reopen with an optional note, and a closed-months list with a "Changed since close" badge. Soft lock: staff are blocked outright, the owner writes with a logged reason and the month goes `dirty`. Current month is not offerable; lines scoped by `transaction_date`, not the statement's period. **Close-time snapshot** (migration `083`, resolves FINANCIAL_AUDIT **F3**): one frozen row per account, written in the same transaction as the lock; P&L and balance sheet serve the sealed figures for a closed month with `view=live` and a stated drift. Exact month ranges only; mid-month balance sheets and pre-snapshot months serve live; only P&L + balance sheet consult snapshots. **Year-end close** (resolves **F4**): one 31 December entry moves the result into Retained Earnings — which is also the account partner profit distribution had always drawn on empty. The closing entry is excluded from the P&L but deliberately **not** from `unclosed_net_income`. Requires December closed; voiding it reopens the year.
- **Card-clearing residual aged at month end (2026-07-27)** — flagging any non-zero clearing balance would have warned every single month, because the last Friday–Sunday of card sales is always still with the acquirer on the 30th. The residual is now aged against month end under FIFO: fresh money reads as normal, money older than four days is flagged with its oldest date — the 184k signature.
- **"Income to bank" — the inflow catch-all (2026-07-27)** — outflows always had "Expense from bank"; inflows had none, so bank interest, supplier refunds and insurance payouts could not be classified at all and the account never reached Reconciled. Posts Dr bank / Cr chosen **revenue** account (crediting an expense would record a refund; enforced server-side). No first-in-list default — that would be Sales Revenue. Also fixed `correct_statement_line` silently dropping `income_account_id`.
- **Cash-flow category override (2026-07-27, FINANCIAL_AUDIT F5)** — migration `084` adds nullable `journal_entries.cash_flow_category`; a manual loan repayment is no longer filed as operating. A junk value is ignored rather than trusted. By-source rows re-keyed on **(source, category)** — with overrides one source can land in two categories, and a source-only key let the last entry seen relabel the rest. **No create-journal form exists in the frontend, deliberately** — a free-form double-entry screen is the easiest way to unbalance a subledger against its control account. Wire the picker if that form is ever built.
- **Late-night date hint (2026-07-27, FINANCIAL_AUDIT F6)** — between midnight and 04:00, a date field sitting on today's date says which date it will use and offers last night's instead. Self-suppresses once another date is picked. No behaviour change: the frontend already sent browser-local dates.
...
## 🏦 BSF — Bank-statement-first workflow  *(owner-driven; money-critical)*

**Owner's real workflow (2026-07):** records payments from the bank statement as they land — often **before** the invoice is uploaded, in **partial** amounts, **days/weeks** later. Suppliers run on a **cari hesap** (running balance): invoices increase what's owed, payments decrease it — no payment-to-invoice matching. Bank charges should just post themselves.

**What already works (verified — do NOT rebuild):** running supplier balance; partial payments of any amount/date post via `post_supplier_payment` and reduce the balance; a learned supplier rule auto-posts future payments; invoice posting nets against payments automatically. Auto-apply already POSTS a new payment when none pre-exists (not link-only).
