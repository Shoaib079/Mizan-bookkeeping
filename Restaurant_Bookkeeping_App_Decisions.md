...
- **Read a tip from an expense photo (owner ask 2026-06-24).** The owner can upload an expense/receipt photo; the app reads the **tip** off it (Turkish `Bahşiş`/`Servis` or English `Tip`/`Gratuity`) and prepares a tip expense (`Dr Tips Expense / Cr Cash`) in the **Needs Review** queue — nothing posts until the owner confirms. The read amount, the cash account it was paid from, and the date are all editable on review. If no tip is found, an empty tip draft is created for the owner to fill in. Re-uploading the same photo for the same restaurant is rejected so a tip is never recorded twice. (Reading the rest of the receipt — supplier, total, line items — stays the normal manual-expense flow; this feature is just the tip.)
