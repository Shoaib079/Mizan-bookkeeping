...
- **Tips — an EXPENSE paid from cash (owner decision 2026-06-23).** Sales are recorded **gross** (the POS system receipt has no separate tip line). When staff are tipped, it is taken from the cash drawer and recorded on the **expense list** like any other line (`Dr expense / Cr cash`). A tip left on a card is still paid to staff from cash and booked on the expense list. There is **no "tips owed to staff" pot / Tips Payable liability** and **no tip logic at POS confirm** — the app never derives or posts tips from the Z report.
- **Card-terminal Z report — per restaurant, match-or-review (owner decision 2026-06-24; simplified 2026-06-24).** Optional per-entity (`card_tips_z_report_enabled`, default off). When enabled, the owner enters the card-terminal **Z report** total with the POS daily summary. The app compares **Z to the system card sale**: **match → post** cash + card as on the system slip; **mismatch → Needs Review** so the owner corrects figures. Tips implied by a Z/card gap are **not** auto-booked — record them on the expense list. Card clearing (`1400`) is debited the **system card sale**; bank deposits + the commission sweep clear it.
  - **Operator note (Z > system card — card tips on terminal):** On mismatch, the Needs Review message explains the day workflow: on re-confirm, **reallocate cash→card** so card equals Z **without changing the daily total** (e.g. Z is 10 TL above system card → lower cash by 10 TL, raise card by 10 TL); **record that tip amount on the expense paper** (`Dr 5700 / Cr cash`); then confirm again. Bank deposit + commission sweep still clear `1400` to zero.
