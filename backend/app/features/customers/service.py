...
from app.features.customers.forex_only_payment_flow import (
    forex_overpayment_warning,
    try_record_forex_only_customer_payment,
)
...
def _customer_entry_reads(
    ...
    # Restore the money account each payment used, so the edit form reopens with
    # the recorded account (cash as cash, USD wallet as USD).
    ...
    payment_je_ids = [
        r.journal_entry_id
        for r in reads
        if r.movement_type == CustomerMovementType.PAYMENT_RECEIVED
...
def record_customer_payment(
    ...
    warning = forex_overpayment_warning(session, entity_id, customer_id, payload)
    forex_response = try_record_forex_only_customer_payment(
        session, entity_id, customer_id, payload, warning=warning
    )
    if forex_response is not None:
        return forex_response
...
def correct_customer_write_off_entry(
    session: Session,
    entity_id: uuid.UUID,
    customer_id: uuid.UUID,
    journal_entry_id: uuid.UUID,
    ...
    *,
    ...
    from app.core.ledger.correction import correct_customer_write_off
    from app.core.receivables.types import CustomerMovementType
    ...
    result = correct_customer_write_off(
        session,
        entity_id,
        journal_entry_id,
        write_off_date=payload.write_off_date,
    ...
    balance = receivables_ledger.current_balance_kurus(session, entity_id, customer_id)
    return CustomerWriteOffResponse(
        journal_entry_id=result.corrected.id,
        balance_kurus=balance,
    )
