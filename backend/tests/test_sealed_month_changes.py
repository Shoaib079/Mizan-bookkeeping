...
def _close_june(db_session, books):
    """Returns the lock ID, not the ORM object.

    Every test here posts something *after* the close, and each post commits.
    A commit expires loaded instances, so touching `lock.id` afterwards makes
    SQLAlchemy reload the row — outside an entity context, where RLS hides it.
    The reload finds nothing and raises ObjectDeletedError, which reads like
    the lock was deleted when it is simply invisible. Read the id while it is
    still valid and carry the UUID instead.
    """
    lock = close_period(
        db_session,
        books["entity_id"],
...
def test_a_month_nobody_touched_lists_nothing(db_session, books):
    ...
    lock_id = _close_june(db_session, books)

    result = _changes(db_session, books, lock_id)
...
def test_entries_from_before_the_close_are_not_changes(db_session, books):
    ...
    assert _changes(db_session, books, lock_id).entries == []
...
def test_an_entry_is_never_listed_twice(db_session, books):
    ...
    lock_id = _close_june_with_activity(db_session, books)
    posted_id = _sale(
