...
const source = () => sourceDeclaring("SidebarNav");

describe("sidebar-nav rendering (IA v2)", () => {
  it("renders overview hub intents as direct links below Dashboard", () => {
    expect(source()).toContain('item.href !== "/"');
    expect(source()).toContain("hubItems.map");
    expect(source()).toContain("NavRowLink");
  });

  it("renders labelled flat groups — no nesting, no accordion controls", () => {
