...
## 2026-08-04 — Manual backup to Cloudflare R2

**Choice:** Settings → **Backup now** (owner/admin) enqueues the same `run_backup()` pipeline as the nightly job and uploads immediately to Cloudflare R2 via the Celery worker. If any successful backup artifact already exists for that **UTC calendar day**, the nightly task **skips create/upload** and only runs retention prune. No restore UI.
...
**Choice:** `bank_statements` + `bank_statement_lines` (entity RLS). CSV/Excel format: `transaction_date` (YYYY-MM-DD), signed `amount` in lira TRY (Turkish format, parsed to integer kuruş), `description`, optional `reference`. Parsers in `adapters/bank_parsers/csv_simple.py` and `xlsx_simple.py` with shared lira→kuruş helper. Import rejects duplicate `(entity_id, file_fingerprint)` and overlapping periods for the same bank money account. Classify `supplier_payment` on outflows: first match existing `SupplierLedgerEntry` (`movement_type=payment`, same supplier, `abs(amount)`, exact date); if found → `status=linked` with FKs, no GL post; else → `post_supplier_payment()` with `payment_account_id` = money account GL sub-account and `reference_type=bank_statement_line`. `bank_fee` / `unknown` set `status=classified` only (no GL this slice). Posted/linked lines cannot be re-classified.
