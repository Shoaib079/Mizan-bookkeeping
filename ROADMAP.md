...
3. ~~**Run pending migrations before go-live**~~ **Superseded — Railway runs `alembic upgrade head` pre-deploy on every push.** Head is **`084_journal_cash_flow_category`** (2026-07-27). Single-head is checked before each commit; two heads once slipped through and had to be untangled by hand.
...
| -------------------------------------------------------------------------------- | ------------------------------------------------- | -------------- | -------------------------------------------------------------------------------------------------------------------------- |
| Statement import wizard mobile (mapping first; sticky Import)                    | `v0.statement-import-mobile`                      | pending tag    | Preview-above-mapping on phone; bury Import after the table; tiny assign chips under 819 |
...
| Partner summary stickers (`v0.partner-page-stickers`)                            | removed by owner 2026-08-21                       | removed        | Re-add gross/undated partner summary panels. Any partner breakdown must be live-derived, dated, and netted (see DECISIONS) |
...
| BSF-2 supplier advances (pay-first, negative AP)                                 | `77f9311` / `v0.bsf-2-advance-import-guard`     | done           | Re-block overpayment in `post_supplier_payment`; import auto-apply respects advance confirm threshold                     |
| BSF-3 supplier name suggestion on import/classify                                | `v0.bsf-3-supplier-suggest`                       | done           | Re-build fuzzy supplier match; duplicate `supplier_suggest.py`                                                           |
| BSF-4 per-supplier auto-post payments toggle                                     | `v0.bsf-4-supplier-auto-post`                     | done           | Re-add `auto_post_payments` column; duplicate trusted import auto-post hook                                              |
...
| Off-site upload storage (S3/R2)                                             | `6ea491c`                                             | done           | Re-store uploads only on ephemeral disk; omit UPLOAD_STORAGE=s3 on Railway |
...
| Partner pay profit (cash / bank)              | done   | Allocate stays books-only; **cash** Pay profit (drawer); **bank** statement classify `partner_profit_paid`; unpaid cap; tag `v0.partner-splits` |
...
| Slice                         | Status | Notes                                                                                                                                                                                                                                                                                  |
| ----------------------------- | ------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Roles & permissions           | done   | `users` + `entity_memberships`; `EntityRole` enum; extensible `Permission` layer; **`grants` JSONB per membership (Option B, migration `088`, 2026-08-03)**; canonical catalog `core/auth/grants.py` ↔ `frontend/src/lib/member-grants.ts`; Settings **Edit access** UI; `X-User-Id` v1 transport; `AUTH_ENFORCEMENT` flag (default off); financial reports guarded; membership CRUD + PATCH grants; Alembic `035` + `088`; 41+ roles pytest |
...
- **Custom member access (Option B) — DONE (2026-08-03).** Owner edits per-member grants in Settings; role presets are shortcuts only. Owner access is fixed full access (not editable). See Decisions §18, migration `088`, tag `v0.custom-member-access`.
...
| **S8** | Shared Excel finish for subledger / activity / delivery / POS | high | **DONE** `v0.s8-shared-excel-finish` — `write_header_row` + `finish_data_table` (no autosize-only bypass) |
...
| 2026-09-01 | Bounce fee selection + manual entry               | `v0.statement-bounce-fee-selection`                    | Checkbox unposted fee lines or manual net fee; mutual clear; `manual_net_fee_kurus` API; rejects posted/mixed modes |
| 2026-09-01 | Bounce fee candidate filter                       | `v0.statement-bounce-fee-filter`                       | Fee/refund lines only in net-fee picker; ücret iadesi; excludes settlements; backend validation |
| 2026-09-01 | Statement rule auto-post conflict fix             | `v0.statement-rule-auto-post-conflict`                 | Best learned rule wins on import; conflict no longer blocks HIGH auto-post; low confidence still → Needs Review |
| 2026-09-01 | Universal payment bounce                          | `v0.statement-bounce-universal`                        | Any line state; auto-void posted/linked/orphan payments; `auto_void_confirmed` API; prepare + payments modules; 7 pytest + vitest |
| 2026-09-01 | Bounce net fee                                    | `v0.statement-bounce-net-fee`                          | Net fee combines charges+refunds; `fee_line_ids` API; zero net settles without GL; `statement_bounce_fees.py`; 3 pytest + vitest |
| 2026-09-01 | Bounce trigger from return lines                  | `v0.statement-bounce-return-trigger`                   | Payment bounced… on positive inflows; dialog picks matching outflow; bounceOutflowCandidates; vitest |
| 2026-09-01 | Card commission period sales                      | `v0.card-commission-period-sales`                      | Clear commission implied-rate uses month-scoped card-sales API; refetch on date change; 7 vitest |
...
| 2026-08-26 | Record icon tiles                               | `v0.record-tile-size`                                 | Icon rail lg:w-48→lg:w-56; tiles p-4 gap-4 min-h ~84px; IconSquare xl (size-12 / size-6) |
...
| 2026-08-25 | Split command palette (file size)               | `v0.fe-split-command-palette`                          | Component 436→~29; hook + panel + row helpers + types; reuse palette-search; guards → sourceDeclaringAll; redeemed from baseline; no behavior change |
| 2026-08-25 | Split customer-payment form (file size)         | `v0.fe-split-customer-payment`                         | Form 446→~81; hook + fields; shared `paymentReceiveAccountLabel` (deduped with correct form); idempotency surface → hook; overpayment guard → sourceDeclaringAll; redeemed from baseline; no behavior change |
...
| 2026-08-24 | Split DateInput (file size)                     | `v0.fe-split-date-input`                               | Component 517→~304; calendar + layout + types; reuse dates/open helpers; dismiss hook gains portalRef (date-input + combobox); redeemed from baseline; no behavior change |
| 2026-08-24 | Split general-ledger panel (file size)          | `v0.fe-split-general-ledger`                           | Panel 538→~91; hook + filters + table; reuse export/detail/GlEntryActions; focus/`effective_only` preserved; redeemed from baseline; no behavior change |
| 2026-08-24 | Split group-sale form (file size)               | `v0.fe-split-group-sale`                               | Form 580→~93; hook + line helpers + header/lines/footer; reuse menu picker + form-copy; idempotency surface → hook; redeemed from baseline; no behavior change |
| 2026-08-24 | Split cash-drawer close-day form (file size)    | `v0.fe-split-cash-close-day`                           | Form 589→~129; hook + phase types + form body + done panel; reuse denom/MainTill/split; guards → sourceDeclaringAll; redeemed from baseline; no behavior change |
| 2026-08-24 | Split nav-sections (file size)                  | `v0.fe-split-nav-sections`                             | Module 608→~40 barrel; types + sections data + route registry + path helpers; public `@/lib/nav-sections` API unchanged; redeemed from baseline; no behavior change |
| 2026-08-24 | Split statement-line review row (file size)     | `v0.fe-split-statement-line-review`                    | Row 632→~126; hook + header/actions/correct dialog; hydrate-once + picker defaults preserved; redeemed from baseline; no behavior change |
| 2026-08-24 | Split statement-classify bar (file size)        | `v0.fe-split-statement-classify`                       | Bar 770→~279; hook + target control + correct dialog; reuse `buildClassifyLinePayload`; guards retargeted; redeemed from baseline; no behavior change |
...
| 2026-08-24 | Split statement-import panel (file size)        | `v0.fe-split-statement-import`                         | Panel 889→115; hook + submit helper + pick/preview/sidebar; redeemed from FILE_SIZE_BASELINE; no behavior/guard change |
| 2026-08-24 | Full frontend audit (discovery)                 | `v0.frontend-audit`                                    | Docs-only: `FRONTEND_AUDIT_REPORT.md` — apiFetch inventory, 25 oversized files, preview/orphan suspects, duplicate chip/download/cash-close patterns; green light; no app/guard changes |
| 2026-08-24 | Sales page visuals (KPI + colors + chip)        | `v0.sales-page-visuals`                                | Posted StatCard KPIs via sales-summary; tfoot + cell colors; FilterChips filled primary; summary above filters |
| 2026-08-24 | Sales period chips (Posted)                     | `v0.sales-period-chips`                                | This month / Last month / Custom below status chips; full prior month; Custom → ReportDateRange; queues still ignore dates; no 7/30-day chips |
| 2026-08-24 | Remove Sales summary UI + Download icon         | `v0.sales-summary-ui-off`                              | Drop SalesSummaryBlock from /sales; shared DownloadIcon=ArrowDownToLine; backend API kept; mutations block-back / tray-icon → red |
...
| 2026-08-24 | v2 rollout + dashboard as-of trim               | `v0.v2-rollout`                                        | Default theme v2; drop This period/range/header date; chart MTD caption; grace toggle to v1 *(toggle removed by v0.v2-only)* |
| 2026-08-23 | Dashboard Cash & bank v2 parity                 | `v0.dashboard-cash-bank-v2-parity`                     | Remove v2-only Cash & bank layout fork; shared two-row total both themes; mutation inline/dividers → red |
...
| 2026-08-23 | Dashboard v2 header                             | `v0.dashboard-v2-header`                               | v2 greeting+name + restaurant colour-dot chip + date range right; ThemeV2Only; v1 PageHeader unchanged; shared theme subscribe; mutation v1 leak / duplicate heading → red |
...
| 2026-08-22 | Sandbox-wide v2 look                            | `v0.sandbox-wide-v2`                                   | Env-gated `data-theme=v2` on `<html>` + owner New look toggle (localStorage); prod unset → v1; display-only |
...
| 2026-08-22 | GS-FX forex-only group sales                    | `v0.gs-fx-forex-only-group-sales`                      | Rateless forex AR subledger-only; zero-cost wallet receipt; TRY at conversion; void/correct; mutation no-GL + zero-cost → red |
...
| 2026-08-21 | S8 shared Excel finish (subledger/activity/delivery/POS) | `v0.s8-shared-excel-finish`                    | Replaced autosize-only with finish_data_table + money_cols; mutation-checked wrong header_row and dropped money_cols |
...
| 2026-08-03 | Custom member access (Option B)              | `v0.custom-member-access`                              | `entity_memberships.grants` JSONB + migration `088` · grant catalog backend/frontend · Settings Edit access · PATCH grants · `/members/me` returns grants · nav/Record/API guards grant-based · role presets · owner access immutable · 43 pytest + 733 vitest |
| 2026-08-03 | Team list scoped to active restaurant        | `v0.team-entity-scope`                                 | Explicit entity_id filter on list members API · TeamPanel client filter + entity-switch reset · useEntityList clears stale rows · pytest + vitest |
| 2026-08-03 | Owner entity switch fix                      | `v0.owner-entity-switch-fix`                           | Do not lock switch before membershipSettled · owners use full entities list in menu/mobile/profile · fixes owner blocked from switching restaurants |
...
| 2026-08-03 | C4 mobile shell (phone UX)                   | `v0.c4-mobile-shell`                                 | Bottom tabs + FAB · `/more` + `/balances` · mobile top bar · More/Settings hub · card rows · transaction bottom sheet · Record quick chips · reports sticky period + Download · void/period-unlock as mobile sheet · unsaved-changes guard on edit forms · no future dates + date-picker fixes · month-to-date default ranges · archived `docs/archive/ux/MOBILE_UX_PLAN.md`; 716 vitest green |
...
| 2026-07-07 | Statement correct after staff void              | `v0.statement-correct-after-staff-void`              | Staff void unlinks linked bank statement lines; Correct skips void when journal already voided then re-posts; AlreadyVoidedError → 409; pytest for void+Correct path; api.ts CORS hint |
| 2026-07-07 | Staff salary — period required (no legacy drawer path) | `ad084e6`                                        | `StaffPaymentCreate` requires `period_year`/`period_month`/`period_salary_minor`; `record_payment` always uses `post_period_salary_payment` (removed amount-only `post_salary_payment` API branch); drawer + statement already send period; pytest rejects no-period POST |
| 2026-07-07 | Statement posted-line audit + party/employee ids | `v0.statement-posted-line-audit`                       | Posted lines: read-only audit bar (no auto-open pickers / first-supplier trap); `initialClassificationForLine` keeps saved type; Correct opens salary period dialog; migration `078` `employee_id`/`partner_id` on statement lines + ledger backfill; resolve on read; pytest + vitest |
...
| 2026-07-07 | Group sale ledger row → detail page | `v0.group-sale-ledger-link` | Customer ledger exposes `reference_type`/`reference_id`; "Group sale" (`credit_sale`) rows link to the group-sale detail page where full structured Edit + Void (menu lines, FX, payment-linked guard) already live |
| 2026-07-07 | Customer payment FX correction — native qty | `v0.customer-payment-fx-correct-native` | `correct_customer_payment` persists `forex_currency` + `payment_native_quantity`; reverses/re-posts FX wallet RECEIPT on void-and-reenter; service passes `payment_native_quantity`; pytest |
| 2026-07-07 | Customer payment FX rate + ledger edit | `v0.customer-payment-fx-rate` | Rate × USD amount auto-fills TRY book value (record + correct forms); fix `payment_received` Actions column so Edit/Void show on customer ledger |
| 2026-07-07 | Partner loan + capital statement classify | `v0.partner-loan-capital-statement` | Rename loan_receipt → "Loan received"; add partner_capital_contribution (Dr bank / Cr 3300), partner_loan_receipt/payment (Cr/Dr 2200 per partner); generic loan for bank/friend |
| 2026-07-07 | Review expenses — expense items tab | `v0.review-expense-items` | **Expenses \| Items** sub-tabs on `/review/expenses`; item picker filters expense list (`expense_item_id`); Items tab shows posted spend per item (time-series) + owner merge; `/expenses/items` → redirect; ⌘K item → filtered expenses; period/item/posted total top-right |
| 2026-07-07 | Statement bulk checkbox UX | `v0.statement-bulk-checkbox-ux` | Always-visible tick next to date + header select-all; per-line post/correct eligibility fixes disabled checkboxes on Posted tab |
...
| 2026-07-05 | P8 — Store / grocery card spend                 | `v0.p8-store-purchase` / `6eba5d1`                     | `store_purchase` classify + learn; retail chain detect; import suggestion; HIGH-confidence auto-post; sign-hint exempt; manual bank/card entry; migration `074`; 6 pytest |
...
| 2026-07-05 | BSF-2 — Supplier advances (pay-first)           | `77f9311` / `v0.bsf-2-advance-import-guard`            | Allow negative AP = advance; soft confirm above threshold (`supplier_advance_confirm_threshold_kurus`, default ₺1k); import auto-apply respects threshold; manual classify still skips confirm; UI labels; 10+ pytest |
...
| 2026-07-05 | Delivery money-flow audit + SEC-2 actor fix   | `v0.delivery-money-flow-sec2`                          | Getir/YS commission upload platform match by VKN; link-platform upgrades kind; delivery settlement inflow needs_review unblock + learned-platform auto-create; `OptionalActorId` empty-string coercion + dev actor fallback; 944+ pytest |
...
| 2026-07-03 | Supplier invoice number uniqueness + void UX   | `v0.73.49-invoice-number-uniqueness`                    | Live posted guard `(supplier, invoice_number)` on post/upload/auto-post; `duplicate` status wired; edit exempt; activity İptal rows + live totals; 6+3 pytest                                                                                              |
| 2026-07-03 | Invoice auto-post on upload (IC-E slice 2)      | `v0.73.48-invoice-supplier-auto-post`                   | Entity setting `invoice_supplier_auto_post`; supplier + classification + expense HIGH → confirm+post on upload; `RULE_AUTO` + `posted_by_rule_auto`; 3 pytest in `test_invoice_auto_post.py`                                                              |
| 2026-07-03 | Invoice one-click post (IC-E slice 1)           | `v0.73.46-invoice-one-click-post`                       | `POST confirm-and-post` when classification + expense account HIGH; `one_click_post_eligible` on draft GET; atomic confirm+post; 20 pytest in `test_invoice_posting.py`                                                                                    |
| 2026-07-03 | Supplier invoice expense-account learning       | `v0.73.44-invoice-supplier-expense-learning`           | Migration `066` `supplier_expense_account_rules`; learn on post; HIGH preselect on draft review; RLS fix `v0.73.45`                                                                                                                                          |
