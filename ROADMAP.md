...
2. **Run pending migrations before go-live:** `alembic upgrade head` applies through **`058`** (`052`–`058`: import profiles, CSV options, classification rules, rule auto-apply, entity `legal_name`, expense item default account, entity **`vkn`**). Ensure `xlrd` installed.
