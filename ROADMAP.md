...
| ---------- | ----------------------------------------------- | ------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-07-07 | Staff salary — period required (no legacy drawer path) | *(uncommitted)*                                  | `StaffPaymentCreate` requires `period_year`/`period_month`/`period_salary_minor`; `record_payment` always uses `post_period_salary_payment` (removed amount-only `post_salary_payment` API branch); drawer + statement already send period; pytest rejects no-period POST |
