...
- **Expense account picker shows GL codes (5000/5200/5210) — show names instead** _(found in real-use testing 2026-06-29; quick fix, not yet built)._ The manual-expense dropdown (and `correct-expense`, `expense-receipt-upload`, `partner-expense-fronted`, `fx-expense-spend`) renders options as `{code} — {name}`, leading with the GL code — unreadable for non-accountants. Change to lead with the account NAME, **prefer the Turkish name** (`name_tr`, e.g. "Kira Gideri"), code in parentheses → e.g. "Kira Gideri (5000)". Expose `name_tr` in the account listing if not already returned. Display-only; the submitted value (account id/code) is unchanged. ~10-minute change.
- **Auto-categorize manual expenses from free text (AI + learning)** _(owner request 2026-06-29)._ Owner types a description ("rent", "peynir/paneer/payneer", "repair machinery") and the system suggests the correct expense account. **Suggest-and-confirm** (pre-fill best guess; owner accepts or overrides — never silently posts, for accounting safety). Decide via (a) **learned mappings first** — reuse/extend `expense_items` + `expense_item_aliases` (normalized, per-entity), then (b) **AI fallback** reusing the receipt-vision OpenAI config (`expense_receipt_vision_*`, gpt-4o-mini) to pick the best-fit account from the entity's chart. **Spelling variants/synonyms** ("peynir"/"paneer"/"payneer", TR↔EN) MUST resolve to ONE account + one canonical item — no per-spelling split (accounts are a fixed list; the typed text is just the entry memo/item). On confirm, record description→account/item so it learns (cheaper/faster over time). Build: new backend "suggest expense account" endpoint + learning upsert + migration; frontend debounced type-ahead pre-fill in the expense form. Dependency: an OpenAI API key (small per-expense cost). Money-adjacent → tests + owner sign-off.
