...
| **Last completed slice** | **Custom member access (2026-08-03)** — per-member grant catalog stored on `entity_memberships`; Settings → Edit access UI; backend + frontend enforce same grants; role presets as shortcuts; migration `088`. |
| **Last commit/tag**      | `v0.custom-member-access` |
