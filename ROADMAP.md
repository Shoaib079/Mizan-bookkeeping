...
| 2. Correct / amend operation | done | `correct_journal_entry()` — atomic void + reversal + corrected post in one transaction; `amends_entry_id` / `amended_by_entry_id` links; `LedgerAuditAction.AMEND`; `POST /entities/{id}/ledger/entries/{id}/correct` (manual only — subledger-backed sources return 409); **subledger-safe follow-up:** `app/core/ledger/correction.py` registry + type-specific flows (append reversal subledger row + new corrected row; mutable detail tables updated); dedicated correct endpoints for supplier payment, customer payment, FX purchase; `test_ledger_correct_subledger.py`; 451 pytest |
